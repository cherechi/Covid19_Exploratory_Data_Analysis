...
select Location, 
	date,COALESCE(total_cases,0) as Daily_Total_Cases, 
...
select CovidDeaths.location, 
	Round((Sum(CovidDeaths.new_cases)/cte.Total_Global_Cases),3) *100 as percentage_Covid_Cases
...
select Location,
	population, MAX(cast(total_deaths as int))as Death_Count, MAX((total_deaths/population))*100 as percentage_deaths
from dbo.CovidDeaths
where location = 'Europe' or location = 'Africa' or location = 'North America' 
or location = 'South America' or location = 'Asia' or location = 'oceania'
and continent is null
...
select dea.Location, 
	COALESCE(MAX(cast(vac.people_fully_vaccinated as bigint)),0) as total_Continent_vaccinations
from dbo.CovidDeaths dea
join dbo.CovidVaccinations vac
	on dea.location = vac.location 
	and dea.date = vac.date
